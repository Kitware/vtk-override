...
import numpy as np
from vtkmodules.numpy_interface.dataset_adapter import VTKObjectWrapper, numpyTovtkDataArray
from vtkmodules.vtkCommonDataModel import vtkDataSet, vtkFieldData

from vtk_override.utils._typing import Number
from vtk_override.utils.arrays import FieldAssociation, convert_array, copy_vtk_array
...
class DataSetAttributes(VTKObjectWrapper):
    ...
    >>> from vtk_override.utils.sources import Cube, Plane
    >>> mesh = Cube()
    ...
    def __init__(self, vtkobject: vtkFieldData, dataset: vtkDataSet, association: FieldAssociation):
    ...
    def get(self, key: str, value: Optional[Any] = None) -> Optional[vtk_ndarray]:
    ...
    def __getitem__(self, key: str) -> vtk_ndarray:
        """Implement ``[]`` operator.
    ...
    @property
    def active_scalars(self) -> Optional[vtk_ndarray]:
    ...
    def active_vectors(self) -> Optional[np.ndarray]:
        """Return the active vectors as a vtk_ndarray.
        ...
        if vectors is not None:
            return vtk_ndarray(vectors, dataset=self.dataset, association=self.association)
        return None
    ...
    @property
    def active_t_coords(self) -> Optional[vtk_ndarray]:
        ...
        if t_coords is not None:
            return vtk_ndarray(t_coords, dataset=self.dataset, association=self.association)
        return None
    ...
    def get_array(self, key: Union[str, int]) -> vtk_ndarray:
        ...
        narray = vtk_ndarray(vtk_arr, dataset=self.dataset, association=self.association)
    ...
    def set_array(
        ...
        * :attr:`active_t_coords_name <DataSetAttributes.active_t_coords_name>`
        ...
        ----------
        data : sequence
            A ``vtk_ndarray``, ``numpy.ndarray``, ``list``,
    ...
    def set_vectors(
        ...
        ----------
        vectors : sequence
            A ``vtk_ndarray``, ``numpy.ndarray``, ``list``, or
    ...
    def _prepare_array(
        ...
        if isinstance(data, vtk_ndarray):
            # vtk_ndarray already contains the reference to the vtk object
    ...
    def pop(self, key: str, default=_SENTINEL) -> vtk_ndarray:
    ...
    def items(self) -> List[Tuple[str, vtk_ndarray]]:
        ...
        [('data0', vtk_ndarray([0, 0, 0, 0, 0, 0])), ('data1', vtk_ndarray([0, 1, 2, 3, 4, 5]))]
    ...
    def values(self) -> List[vtk_ndarray]:
        ...
        [vtk_ndarray([0, 0, 0, 0, 0, 0]), vtk_ndarray([0, 1, 2, 3, 4, 5])]
    ...
    def update(self, array_dict: Union[Dict[str, np.ndarray], "DataSetAttributes"]):
        ...
        >>> import numpy as np
        >>> from vtk_override.utils.sources import Wavelet
        >>> mesh = Wavelet()
    ...
    @property
    def active_normals(self) -> Optional[vtk_ndarray]:
        ...
        vtk_ndarray([[0.000000e+00,  0.000000e+00, -1.000000e+00],
        ...
        if vtk_normals is not None:
            return vtk_ndarray(vtk_normals, dataset=self.dataset, association=self.association)
        return None
    ...
    def active_normals(self, normals: Union[Sequence[Number], np.ndarray]):
        ...
        vtkarr = numpyTovtkDataArray(normals, name="Normals")
